  ...
  , mpXunoBrowser(0)
{
    ...
    qDebug("start MainWindow rev.%s%s",__DATE__,__TIME__);
    setWindowIcon(QIcon(":/QtAV.ico"));
...
//    mpTitle->setIndent(8);
    ...
    qDebug("PixMap of buttons: %d x %d", mPlayPixmap.width(), mPlayPixmap.height());
...
*/
    ...
    /*QString tname="http://www.xuno.com/content/RED/intro_10bit/RED_480x206_yuv444p10le.mp4";
    qDebug("MainWindow::setupUi before loadRemoteUrlPresset url: %s",qPrintable(tname));
    loadRemoteUrlPresset(tname);
    qDebug("MainWindow::setupUi after loadRemoteUrlPresset url: %s",qPrintable(tname));
    */
}
...
void MainWindow::play(const QString &name)
{
    ...
    qDebug("MainWindow::play before loadRemoteUrlPresset url: %s",qPrintable(mFile));
    loadRemoteUrlPresset(mFile);
    qDebug("MainWindow::play after loadRemoteUrlPresset url: %s",qPrintable(mFile));
    mpPlayer->play(mFile);
}
...
void MainWindow::setVolume()
{
    ...
    qDebug("MainWindow::setVolume bar:%d, vol: %f",mpVolumeSlider->value(),v);
    if (ao) {
        ao->setVolume(v);
    }
...
{
    mControlOn = ((e->key() == Qt::Key_Control)||(e->modifiers() == Qt::ControlModifier)) ;
}
...
{
    qDebug("MainWindow::openUrl start");
    QString url = QInputDialog::getText(0, tr("Open an url"), tr("Url"));
    if (QUrl(url).isValid()){
      qDebug("MainWindow::openUrl befor play url: %s",qPrintable(url));
      play(url);
    }
    return;
}
...
{
qDebug("MainWindow::onSaturationChanged: %d, requseted: %f", s, mpVideoEQ->saturation());
     VideoRenderer *vo = mpPlayer->renderer();
...
/*
    if (mpVideoEQ->engine() != VideoEQConfigPage::SWScale
            && vo->setGammaRGB(mpVideoEQ->gammaRGB())) {
        mpPlayer->setGammaRGB(mpVideoEQ->gammaRGB());
    } else {
        vo->setGammaRGB(mpVideoEQ->gammaRGB());
        mpPlayer->setGammaRGB(mpVideoEQ->gammaRGB());
    }
...
{
    qDebug("MainWindow::onFilterSharpChanged: %d, requseted: %f", fs, mpVideoEQ->filterSharp());
    VideoRenderer *vo = mpPlayer->renderer();
    vo->setFilterSharp(mpVideoEQ->filterSharp());
    /*
    if (mpVideoEQ->engine() != VideoEQConfigPage::SWScale
            && vo->setFilterSharp(mpVideoEQ->filterSharp())) {
        mpPlayer->setFilterSharp(mpVideoEQ->filterSharp());
    } else {
        vo->setFilterSharp(mpVideoEQ->filterSharp());
        mpPlayer->setFilterSharp(mpVideoEQ->filterSharp());
    }
...
{
    qDebug("MainWindow::handleFullscreenChange");
    workaroundRendererSize();
...
void MainWindow::reSizeByMovie()
{
    ...
    qDebug("MainWindow::reSizeByMovie w:%d  h:%d",t.width(),t.height());
    if (t.isValid() && (!t.isNull())) resize(t);
}

void MainWindow::onXunoBrowser(){
  qDebug("Start onXunoBrowser");
  if (!mpXunoBrowser){
...
void MainWindow::onClickXunoBrowser(){
  ...
  qDebug("MainWindow::onClickXunoBrowser: %s",qPrintable(url.toString()));
  if (url.isValid()) play(url.toString());
}

void MainWindow::onFullScreen(){
 qDebug("MainWindow::onFullScreen");
    if (isFullScreen())
        showNormal();
    else
        showFullScreen();
